...
public sealed class SalesOrderLineEditorForm : Form
{
    private const decimal MinimumLinePrice = 0.01m;
    private readonly SalesOrderLineRecord _draft;
    ...
    public SalesOrderLineEditorForm(
        IReadOnlyList<SalesCatalogItemOption> catalogItems,
        SalesOrderLineRecord? line = null)
    {
        _catalogItems = catalogItems;
        _draft = line?.Clone() ?? new SalesOrderLineRecord { Id = Guid.NewGuid() };
        ResultLine = null;
        ...
        MaximizeBox = false;
        MinimizeBox = false;
        Text = line is null ? "Новая позиция" : "Позиция заказа";
    ...
    private void BuildLayout()
    {
        ...
        _quantityNumeric.Minimum = MinimumLinePrice;
        ...
        _priceNumeric.Minimum = MinimumLinePrice;
    ...
    private void LoadDraft()
    {
        ...
        {
            var selected = _catalogItems.FirstOrDefault(item => item.Code == _draft.ItemCode);
    ...
    private void ApplySelectedItemDefaults()
    {
        ...
        if (_draft.ItemCode != item.Code)
        {
            _priceNumeric.Value = item.DefaultPrice > 0
                ? item.DefaultPrice
                : MinimumLinePrice;
        }
    ...
    private void SaveAndClose()
    {
        ...
        if (item is null)
        {
            MessageBox.Show(this, "Выберите номенклатуру.", "Позиция заказа", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }

        _draft.ItemCode = item.Code;
        _draft.ItemName = item.Name;
        _draft.Unit = item.Unit;
        _draft.Quantity = _quantityNumeric.Value;
        _draft.Price = _priceNumeric.Value;

        ResultLine = _draft.Clone();
        DialogResult = DialogResult.OK;
    }

    private SalesCatalogItemOption? ResolveSelectedItem()
    {
        if (_itemComboBox.SelectedItem is SalesCatalogItemOption selected)
        {
            return selected;
        }
        ...
        var match = _catalogItems.FirstOrDefault(item =>
        ...
        {
            _itemComboBox.SelectedItem = match;
        }
