...
public sealed class SalesShipmentEditorForm : Form
{
    private readonly SalesWorkspace _workspace;
    private readonly SalesInventoryService _inventory;
    private readonly SalesShipmentRecord _draft;
    ...
    private readonly DateTimePicker _shipmentDatePicker = new();
    ...
    private readonly TextBox _carrierTextBox = new();
    ...
    public SalesShipmentEditorForm(SalesWorkspace workspace, SalesShipmentRecord shipment)
    {
        _workspace = workspace;
        _inventory = new SalesInventoryService(workspace);
        _draft = shipment.Clone();
        ...
        MaximizeBox = false;
        MinimizeBox = false;
        Text = string.IsNullOrWhiteSpace(shipment.Number)
            ? "Расходная накладная"
    ...
    private Control CreateHeader()
    {
        ...
        {
            Text = "Документ отгрузки собирает склад, перевозчика и состав строк в одном окне.",
            Dock = DockStyle.Top,
            Height = 22,
    ...
    private Control CreateHeaderFields()
    {
        ...
        _statusComboBox.Items.AddRange(_workspace.ShipmentStatuses.Cast<object>().ToArray());
        ...
        grid.Controls.Add(CreateFieldPanel("Дата отгрузки", _shipmentDatePicker), 1, 0);
        grid.Controls.Add(CreateContextPanel("Основание", _orderValueLabel), 2, 0);
        grid.Controls.Add(CreateContextPanel("Покупатель", _customerValueLabel), 0, 1);
        grid.Controls.Add(CreateFieldPanel("Склад", _warehouseComboBox), 1, 1);
        grid.Controls.Add(CreateFieldPanel("Статус", _statusComboBox), 2, 1);
        grid.Controls.Add(CreateFieldPanel("Перевозчик", _carrierTextBox), 0, 2);
    ...
    private Control CreateLinesSection()
    {
        ...
        {
            Text = "Сотрудник видит весь состав отгрузки и может сразу проверить складской документ перед печатью.",
            Dock = DockStyle.Top,
            Height = 22,
    ...
    private Control CreateButtons()
    {
        ...
        var saveButton = CreateActionButton("Сохранить отгрузку", Color.FromArgb(242, 194, 89), Color.FromArgb(42, 36, 29));
    ...
    private void LoadDraft()
    {
        ...
        _shipmentDatePicker.Value = _draft.ShipmentDate == default ? DateTime.Today : _draft.ShipmentDate;
        _statusComboBox.SelectedItem = string.IsNullOrWhiteSpace(_draft.Status) ? _workspace.ShipmentStatuses.First() : _draft.Status;
        ...
        _carrierTextBox.Text = _draft.Carrier;
    ...
    private void SaveAndClose()
    {
        ...
        {
            ShowValidationError("Укажите номер отгрузки.");
            return;
        }

        if (_statusComboBox.SelectedItem is null || _warehouseComboBox.SelectedItem is null || _managerComboBox.SelectedItem is null)
        {
            ShowValidationError("Заполните статус, склад и менеджера.");
            return;
        }

        if (_lines.Count == 0)
        {
            ShowValidationError("В отгрузке должна быть хотя бы одна позиция.");
            return;
        }

        var stockCheck = RefreshStockStatus();
        if (!stockCheck.IsFullyCovered
            && _statusComboBox.SelectedItem?.ToString() is string status
            && !status.Equals("Черновик", StringComparison.OrdinalIgnoreCase))
        {
            ShowValidationError("На выбранном складе не хватает остатка. Оставьте документ в черновике или смените склад.");
            return;
        }

        _draft.Number = _numberTextBox.Text.Trim();
        _draft.ShipmentDate = _shipmentDatePicker.Value.Date;
        _draft.Status = _statusComboBox.SelectedItem as string ?? string.Empty;
        _draft.Warehouse = _warehouseComboBox.SelectedItem as string ?? string.Empty;
        _draft.Manager = _managerComboBox.SelectedItem as string ?? string.Empty;
        _draft.Carrier = _carrierTextBox.Text.Trim();
        ...
        ResultShipment = _draft.Clone();
        DialogResult = DialogResult.OK;
    }

    private SalesInventoryCheck RefreshStockStatus()
    {
        if (_lines.Count == 0)
        {
            _stockStatusValueLabel.Text = "Остаток не рассчитан";
            _stockStatusValueLabel.ForeColor = Color.FromArgb(165, 110, 47);
            _stockHintValueLabel.Text = "В документе нет строк для проверки доступности на складе.";
            return new SalesInventoryCheck(false, _stockStatusValueLabel.Text, _stockHintValueLabel.Text, Array.Empty<SalesInventoryLineStatus>());
        }

        var warehouse = _warehouseComboBox.SelectedItem?.ToString() ?? _draft.Warehouse;
        var shipment = _draft.Clone();
        shipment.Warehouse = warehouse;
        shipment.Lines = new BindingList<SalesOrderLineRecord>(_lines.Select(line => line.Clone()).ToList());
        var check = _inventory.AnalyzeShipment(shipment);
    ...
    {
        MessageBox.Show(this, message, "Расходная накладная", MessageBoxButtons.OK, MessageBoxIcon.Warning);
    }
}
