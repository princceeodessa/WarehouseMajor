...
internal static class OneCTextNormalizer
{
    ...
    public static string NormalizeText(string? value)
    {
        ...
        return NormalizeCandidate(value.Trim());
    }
    ...
    private static string NormalizeCandidate(string original)
    {
        ...
        for (var i = 0; i < 4; i++)
        {
            current = Utf8.GetString(Cp1251.GetBytes(current));
            if (current.Contains('\uFFFD', StringComparison.Ordinal) ||
                CountChar(current, '?') > CountChar(original, '?'))
            {
                break;
            }
